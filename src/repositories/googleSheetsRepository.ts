...
export class GoogleSheetsRepository {
  ...
  constructor (spreadsheetId: string) {
    ...
    this.auth = new google.auth.GoogleAuth({ keyFile: this.keyFile, scopes: this.scopes });
  ...
  async updateFinalMarkExam (finalMark: number[][] | Status[][] ): Promise<void> {
    await this.googleSheet.spreadsheets.values.update({
      auth: this.auth,
      spreadsheetId: this.spreadsheetId,
  ...
  async getStudentsAbsences (): Promise<number[] | null> {
    ...
    const absensesArray = data.reduce((acc, current) => acc.concat(current), []);
